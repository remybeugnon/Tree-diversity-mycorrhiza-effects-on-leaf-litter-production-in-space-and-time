...
# 2025-05-21
# Tukey test: Fig. 2 Yearly, monthly and cumulative litter fall + wood biomass increment (productivity)
...
df.mass.long <- df.mass.long %>% 
  ...
  ungroup() 
  #%>%
  #dplyr::select(-c("month","month1","sr","div","blk"))
...
group_means <- df.monthly.litter %>%
  group_by(month1, myc) %>%
  summarise(mean_litter = mean(litter.prod, na.rm = TRUE), .groups = "drop") %>%
  rename(month = month1)
...
Tukey_results_monthly_lit <- map_df(myc_sig_months, function(mo) {
  dat <- df.monthly.litter %>% filter(month1 == mo)
  
  mod <- lme(litter.prod ~ log2(sr) * myc, random = ~1|composition, data = dat)
...
final_results <- left_join(pairwise_raw_diff, Tukey_results_monthly_lit, by = c("month", "contrast"))

# Export
writexl::write_xlsx(final_results, "C:/Users/eb64wupi/Documents/MyDiv-litterfall-project/1-data/1-5-TukeyTests/Litterfall_Monthly_Tukey_Mycorrhiza_Significant_with_RelDiff.xlsx")
### ! NAs because of missing mycorrhizal type effects




# Get all pairwise combinations of sr groups per month, calculate relative % difference

# Step 1: Calculate group means
group_means_sr <- df.monthly.litter %>%
  group_by(month1, sr) %>%
  summarise(mean_litter = mean(litter.prod, na.rm = TRUE), .groups = "drop") %>%
  rename(month = month1)

# Step 2: Generate pairwise combinations per month
sr_pairwise <- group_means_sr %>%
  group_by(month) %>%
  summarise(pairs = list(combn(sr, 2, simplify = FALSE)), .groups = "drop") %>%
  unnest(pairs) %>%
  mutate(
    sr1 = map_int(pairs, 1),
    sr2 = map_int(pairs, 2)
...
# Step 3: Join to get means for each group
sr_pairwise <- sr_pairwise %>%
  left_join(group_means_sr, by = c("month", "sr1" = "sr")) %>%
  rename(mean1 = mean_litter) %>%
  left_join(group_means_sr, by = c("month", "sr2" = "sr")) %>%
  rename(mean2 = mean_litter)

# Step 4: Compute relative change
sr_pairwise <- sr_pairwise %>%
  mutate(
    rel_change_percent = round(100 * (mean1 - mean2) / mean2, 1),
    contrast = paste0(sr1, " - ", sr2)
  ) %>%
  select(month, contrast, rel_change_percent)
...
ggplot(d.Dec, 
  ...
  geom_smooth(method="lm", alpha=0.3)+
  labs(
    y = expression(atop("Litterfall December", (kg ~ ha^-1))),  # Use atop() for line break
    x = "Tree species richness"
  ) +
  scale_x_continuous(trans='log2',
                     breaks=c(1,2,4))+
...
#---------------------------------------------------------------------
# MyDiv experiment; Litterfall experiment March 2023 - February 2024
# Wood biomass data from Claudia Guimarães-Steinicke (2022) & Tama Ray (2021)
...
emmeans(mod.incr, list(pairwise ~ "myc"), adjust = "tukey")


# Get average biomass per richness level and mycorrhizal type
mean_biomass <- wood_litter %>%
  group_by(sr, myc) %>%
  summarise(mean_incr = mean(increment, na.rm = TRUE), .groups = "drop")

biomass_wide <- mean_biomass %>%
  pivot_wider(names_from = myc, values_from = mean_incr)
...
# Get average biomass per mycorrhizal type
mean_biomass_myc <- wood_litter %>%
  group_by(myc) %>%
  summarise(mean_incr = mean(increment, na.rm = TRUE), .groups = "drop")

biomass_wide_myc <- mean_biomass_myc %>%
  pivot_wider(names_from = myc, values_from = mean_incr)

biomass_wide_myc <- biomass_wide_myc %>%
  mutate(
    EM_vs_AM      = (EM - AM) / AM * 100,
    AMEM_vs_AM    = (`AM + EM` - AM) / AM * 100,
    AMEM_vs_EM    = (`AM + EM` - EM) / EM * 100
...
emmeans(mod.total.bio, list(pairwise ~ "myc"), adjust = "tukey")


# Get average biomass per richness level and mycorrhizal type
mean_biomass_tot <- wood_litter2 %>%
  group_by(sr, myc) %>%
  summarise(mean_tbt = mean(total_bio_tree, na.rm = TRUE), .groups = "drop")

biomass_wide_tot <- mean_biomass_tot %>%
  pivot_wider(names_from = myc, values_from = mean_tbt)

biomass_wide_tot <- biomass_wide_tot %>%
  mutate(
    EM_vs_AM      = (EM - AM) / AM * 100,
    AMEM_vs_AM    = (`AM + EM` - AM) / AM * 100,
    AMEM_vs_EM    = (`AM + EM` - EM) / EM * 100
...
# Get average biomass per mycorrhizal type
mean_biomass_tot_myc <- wood_litter2 %>%
  group_by(myc) %>%
  summarise(mean_tbt = mean(total_bio_tree, na.rm = TRUE), .groups = "drop")

mean_biomass_tot_myc <- mean_biomass_tot_myc %>%
  pivot_wider(names_from = myc, values_from = mean_tbt)
